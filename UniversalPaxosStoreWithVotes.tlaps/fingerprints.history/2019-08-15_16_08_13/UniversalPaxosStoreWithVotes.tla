      ...
      <3>1. ASSUME NEW q \in Participant,
        ...
        <4>1. \E p \in Participant, b \in Ballot, v \in Value : EV!VoteFor(p, b, v)                     
          <5> SUFFICES ASSUME NEW m \in msgs,
                              /\ q \in m.to
                              /\ LET p == m.from
                                 IN  UpdateStateV(p, p, m.state[p]) \* replacing UpdateState
                              (*
                              /\ IF \/ m.state[p].maxBal < state'[p][p].maxBal
                                    \/ m.state[p].maxVBal < state'[p][p].maxVBal
                                 THEN Send([from |-> p, to |-> {m.from}, state |-> state'[p]]) 
                                 ELSE UNCHANGED msgs
                              *)
                       PROVE  \E pp \in Participant, bb \in Ballot, vv \in Value : EV!VoteFor(pp, bb, vv)
...
\* Last modified Thu Aug 15 16:07:27 CST 2019 by hengxin
